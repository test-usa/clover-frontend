...
interface MatchCardProps {
    ...
    verifiedType?: 'blue' | 'orange' | 'none'; // New prop
    ...
    return (
        <div className="bg-white rounded-lg p-4 shadow-sm border min-w-[350px] max-w-[400px] border-gray-200">
            ...
            <div className="flex gap-2 items-center mb-4">
                <div>
                    <img src={avatarSrc} alt={`${name}'s avatar`} className="w-12 h-12 rounded-full mr-3" />
                </div>
                <div className="relative text-start">
                    <h3 className="text-lg font-semibold text-gray-800 flex items-center">
                        ...
                        <div className="absolute -top-2 -left-2">
                            <svg
                                className={`ml-1 w-4 h-4 absolute top-2 -left-6 z-10 ${verifiedColor}`}
                                fill="currentColor"
                                viewBox="0 0 20 20"
                ...
                <div className="ml-auto">
                    <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${status === 'Available' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                            }`}
                    >
                        {status}
                    </span>
